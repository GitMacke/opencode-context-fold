...
Choose the level of detail that serves the current task. Keep low-level detail while working through a problem; once the outcome is understood, fold intermediary steps that no longer contribute to the bigger picture. Preserve what was learned, decided, and remains unresolved. Use peek to return to supporting detail when needed, and fold earlier summaries into a higher-level account as the work progresses.
